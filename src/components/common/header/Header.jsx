...
const Header = ({ props, hnadleclick }) => {
  ...
  const { darkMode, toggleDarkMode, darkMoreMainSection } =
  ...
  const shareURLClick = () => {
    const pageurl =
      process.env.APP_URL +
      (props.editor.editorData ? props.editor.editorData.slug : "");
    if (pageurl) {
  ...
  return (
    <>
      <div
        className={` ${
          isUserRoute ? styles.user_header_wrapper : styles.header_wrapper
        } ${darkMode ? "dark_mode" : ""}`}
        style={
      ...
      >
        <div className="header_section" ref={headerContainer}>
          <div className={styles.header_inner}>
            {!isUserRoute ? (
              ...
              <>
                {/* <Link
                  href={
                    tokenData?.role === 2 ? "/dashboard" : "/admin/dashboard"
                  }
                >
            ...
            {editorHeaderStatus === false ? (
              <>
                <div className={styles.header_nav} ref={headerNav}></div>
                <div className={styles.header_right_action}>
                  <div className={styles.header_dark_mode_icon}>
                    <span
                      className={`${
                        darkMoreMainSection
                          ? `${styles.darkmode_header_dark_mode_icon_initial}`
                          : `${styles.header_dark_mode_icon_initial}`
                      }`}
                    >
                      <FaRegMoon onClick={toggleDarkMode} />
                    </span>
                  </div>
                  <div
                    className={"pu_dropdown_wrapper " + styles.header_profile}
                  >
                    <div
                      className={"pu_dropdown_toggle " + styles.header_avatar}
                      data-toggle="dropdown"
                    >
                      <div className={styles.header_avatar_icon}>
                        <span className={styles.header_avatar_icon_initial}>
                          {getNameInitials(
                            cData.userData ? cData.userData.name : "Unknown"
                          )}
                        </span>
                        {cData.userData.profile_img ? (
                          <img src={cData.userData.profile_img} alt="" />
                        ) : (
                          ""
                        )}
                      </div>
                      <div
                        className={` ${
                          isUserRoute
                            ? styles.header_avatar_details
                            : styles.avatar__details
                        }`}
                      >
                        <p>
                          Hi, <span>{cData.userData.name.split(" ")[0]}</span>
                        </p>
                      </div>
                      <div className={styles.header_avatar_arrow}>
                        ...
                        >
                          <path
                            fillRule="evenodd"
                            clipRule="evenodd"
                            d="M4.46201 4.99102L0.988007 0.986023H7.99701L4.46201 4.99102Z"
                            fill="#9AA4CC"
                          />
                        </svg>
                      </div>
                    ...
                    <div
                      className={"pu_dropdown_dd " + styles.profile_dropdown}
                    >
                      ...
                      <ul>
                        ...
                        {tokenData?.role === 1 ? (
                          <li>
                            ...
                            <Link href={"/admin/frontendSettings"}>
                              <a
                                className="pu_dropdown_link"
                                style={{
                                  textDecoration: "none",
              ...
              <>
                <div className={styles.editor_title}>
                  <span onClick={() => editorHeaderBackBtn()}>
                    <a className="pu_back_arrow">
                      <span className="pu_back_arrow_icon">
                        {svg.back_arrow}
                      </span>
                      {pathname === "/settings/[id]"
                        ? "Back to Editor"
                  ...
                  <p>
                    {props.editor.editorData
                      ? props.editor.editorData.title
                      : "Unknown"}
                  </p>
                </div>
                <div
                  className={
                    styles.header_right_action +
                    " " +
                    styles.editor_header_right
                  }
                >
                  <AutoSaveLoader />
                  {pathname !== "/settings/[id]" ? (
                    <Link href={"/settings/" + tempId}>
                      <a
                        className={
                          "pu_btn pu_btn_border " +
                          styles.pu_btn +
                          " " +
                          styles.pu_btn_settings
                        }
                      >
                        {svg.icon_page_setting} Settings
                      </a>
                    </Link>
                  ) : null}
                  ...
                  <button
                    className={
                      "pu_btn " + styles.pu_btn + " " + styles.pu_btn_share
                    }
                    onClick={() => shareURLClick()}
                  >
                    {svg.icon_share} Share
                  </button>
                  ...
                  <div className={styles.header_right_action_mobile}>
                    ...
                    >
                      {svg.header_icon_link}
                    </a>
                    <div
                      className="pu_btn_icon"
                      onClick={() => shareURLClick()}
                    >
                      {svg.header_icon_share}
                    </div>
                  </div>
                </div>
              </>
