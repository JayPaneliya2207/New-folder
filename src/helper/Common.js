...
export let common = {
      ...
      if (Object.keys(params.data).length) {
        ...
        for (var p in params.data) {
          if (params.data.hasOwnProperty(p)) {
            str.push(
              encodeURIComponent(p) + "=" + encodeURIComponent(params.data[p])
            );
          }
        }
          ...
          if (resp.status == "success") {
            if (params.alert !== false) {
              if (resp.message && resp.message != "") {
                AlertMsg("success", "Congratulations!", resp.message);
              }
            }
  ...
  tableNumCnt: (currentPage, dataPerPage) => {
    ...
    return currentPage == 1 ? 1 : currentPage * dataPerPage - dataPerPage + 1;
  },
...
export let manageMyFile = (params, cb) => {
  ...
  let selFileExt =
    "." + selectedFile.name.split(".").reverse()[0]?.toLowerCase();

  let acceptAry = elements.target.accept
  ...
  if (!acceptAry.includes(selFileExt)) {
    AlertMsg(
      ...
      `Only ${
        acceptAry.join(", ") +
        (acceptFileTypeAry.length == 1 ? "file is " : " files are ")
      } allowed.`
    );
    ...
    if (selFileType.split("audio").length > 1) {
      ...
      selFileType.split("video").length > 1 ||
      selFileType.split("image").length > 1
    ) {
