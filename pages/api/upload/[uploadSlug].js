  ...
  try {
    ...
    if (profileName && user) {
      ...
      if (file) {
        ...
        // 	return;
        // });
        signedurl = await CommonAPI.getPutObjectSignedURL(remoteFile);
        if (signedurl != undefined) {
          if (user.profilePicture && user.profilePicture != "") {
            CommonAPI.deleteObjects([user.profilePicture.key]);
          }
          set.profilePicture = { key: remoteFile, file: remoteFile };
        }
      }
  ...
  try {
    ...
    if (templateId != "" && user) {
      ...
      if (file && file.size) {
        ...
        // 	return;
        // });
        let signedurl = await CommonAPI.getPutObjectSignedURL(remoteFile);
        ...
        if (signedurl != undefined) {
          ...
          if (template && typeof template._id !== "undefined") {
            ...
            if (
              ...
              template.profile.image.indexOf("profileImage") === -1
            ) {
...
async function getNextSequenceValue(sequenceName) {
  ...
  if (sequenceDocument != null) {
    sequence = sequenceDocument.sequence_value;
  } else {
    let create = await AutoIncreament.create({
      _id: "imageId",
      sequence_value: 1,
    });
    console.log("created sequence", create);
    sequence = create.sequence_value;
  }
  ...
  try {
    ...
    if (section_id != "" && user) {
      ...
      if (file && file.size) {
        ...
        let r;
        let signedurl = await CommonAPI.getPutObjectSignedURL(remoteFile);
        // r = await CommonAPI.upload(remoteFile, {
        // 	ContentType: file.mimetype,
        // }).catch((err) => {
        // 	res.json({
        // 		status: "error",
                ...
                for (let i = 0; i < section.otherBusiness.length; i++) {
                  if (imageID == section.otherBusiness[i].imageID) {
                    await CommonAPI.deleteObjects([
                      section.otherBusiness[i].image,
                    ]);
                  }
                }
              }
              imageID = JSON.parse(imageID);
              await sectionTbl.updateMany(where, {
                $pull: { otherBusiness: { imageID: imageID } },
              });
            }
        ...
        await sectionTbl
          .updateMany(
            { _id: section_id, "otherBusiness.imageID": imageID },
            { "otherBusiness.$.url": imageurl }
          )
          .then((result) => {
            res.json({
              status: "success",
            ...
            await templateTbl
              ...
              .then(() => {
                res.json({
                  status: "success",
                  message:
                    "The template Thumb Image has been uploaded successfully.",
                  data: { thumb: thumb },
                  signedurl: signedurl,
                });
              });
