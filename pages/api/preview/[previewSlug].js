...
const updateVisitorAnalytics = async (templatePageData, otherData) => {
  ...
  if (visitDetailsData && typeof visitDetailsData._id != "undefined") {
    ...
    await CampaignVisitDetail.updateOne(
      { _id: visitDetailsData._id },
      {
    ...
    await CampaignVisitDetail.create(visitDetailFields).then(
      (visitDetailresult) => {
  ...
  if (visitPageDetailsData && typeof visitPageDetailsData._id != "undefined") {
    ...
    await CampaignPageVisitDetail.updateOne(
      { _id: visitPageDetailsData._id },
      {
    ...
    await CampaignPageVisitDetail.create(visitPageDetailFields).then(
      (visitPageDetailresult) => {
        //console.log("Create visitDetail");
      }
    );
  }
...
const updateClickAnalytics = async (templatePageData, otherData) => {
  ...
  if (linkData && typeof linkData._id != "undefined") {
    ...
    if (linkDetailsData && linkDetailsData._id != "undefined") {
      ...
      await CampaignLinkDetail.create(linkDetailFields).then(
        (linkDetailresult) => {
...
routeHandler.getTemplatePage = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      if (
        typeof postdata.page_slug != "undefined" &&
        postdata.page_slug != ""
      ) {
        where2 = {
    ...
    if (templateData) {
      if (templateData.profile != "") {
        try {
          const url = await CommonAPI.getObjectSignedURL(
            templateData.profile.image
          );
          templateData.profile.image = url;
        } catch (error) {
      ...
      if (templatePageData && typeof templatePageData._id != "undefined") {
        let pageSections = await templateSectionTbl
          .find({ templateId: templateData._id, pageId: templatePageData._id })
          .sort({ sort: 1 });

        for (let i = 0; i < pageSections.length; i++) {
          if (pageSections[i].otherBusiness.length > 0) {
            for (const [index, element] of pageSections[
              i
            ].otherBusiness.entries()) {
              try {
                const url = await CommonAPI.getObjectSignedURL(element.image);
                pageSections[i].otherBusiness[index].image = url;
              } catch (error) {
                console.error(`Error updating image at index ${index}:`, error);
              }
            }
        ...
        if (
          typeof postdata.isVisitor != "undefined" &&
          postdata.isVisitor == true
        ) {
          if (isAdmin == false) {
        ...
        res.json({
          ...
          data: {
            ...
            isAdmin: isAdmin,
            adEnable: check?.isAdEnabled,
            adScript: check?.adScript,
            adScriptCode: check?.adScriptCode,
            validity: checkExpire?.validityDate,
          },
        });
      } else {
        res.json({
