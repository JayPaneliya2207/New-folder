  ...
  try {
    ...
    if (response.status) {
      ...
      await Users.findOneAndUpdate(
        ...
        if (result.profilePicture != "" && result.profilePicture != undefined) {
          try {
            const url = await CommonAPI.getObjectSignedURL(
              result.profilePicture.file
            );
...
routeHandler.getProfile = async (req, res) => {
  let user = req.vsuser;
  const postData = req.body;
  try {
    if (postData.userId) {
      return await Users.findOne({ _id: postData.userId })
        .then((result) => {
          if (result) {
            res.json({
              status: "success",
    ...
    await Users.findOne({ _id: user._id })
      .then((result) => {
        if (result) {
          res.json({
            status: "success",
  ...
  try {
    ...
    if (postdata.mode == "Bank Transfer") {
      ...
      const encoded = Buffer.from(
        `${paypalCredentials.key}:${paypalCredentials.secretKey}`,
        "utf8"
      ).toString("base64");

      const response = await fetch(
        `${paypalCredentials.apiURL}checkout/orders`,
        {
          method: "POST",
          headers: {
      ...
      try {
        ...
        res.json({
          status: "success",
          message: "Razorpay checkout",
          data: checkoutData,
          type: "Razorpay",
          queryParameter: querystr,
        });
      } catch (error) {
        console.error(error);
...
routeHandler.paymentSuccess = async (req, res) => {
  try {
    ...
    if (postdata?.token) {
      ...
      const response = await fetch(
        `${paypalCredentials.apiURL}checkout/orders/${token}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Basic " + encoded,
          ...
          if (userdata) {
            if (
              ...
              if (userdata.validityDate && userdata.validityDate !== "") {
                ...
                await Users.findOneAndUpdate(
                  { _id: userid },
                  { validityDate: newDate, planName }
                );
              } else {
                validityDate = addDays(new Date(), validity);
                await Users.findOneAndUpdate(
                  { _id: userid },
                  { validityDate: validityDate, planName }
                );
              }
            }
          }
...
routeHandler.downloadInvoice = async (req, res) => {
  ...
  try {
    ...
    if (orderData) {
      ...
      let fileName =
        "invoice_" + orderData.id.substring(orderData.id.length - 5) + ".pdf";
      ...
      const currencyIndex = currencies.findIndex(
        (row) => row.code == orderData.currency.toUpperCase()
      );
      ...
      await page.pdf({
        path: `${process.env.folderPath}/public/${filePath}`,
        format: "A4",
      });
      await page.close();
      await browser.close();
...
routeHandler.getUserDashboardAnalytics = async (req, res) => {
  let postdata = req.body;
  let user = req.vsuser;
  try {
    ...
    data.fundWalletBalance =
      Common.getWalletBalance(walletSettingTable, userWallet, "fund_wallet") ||
      0;
    data.mainWalletBalance =
      Common.getWalletBalance(walletSettingTable, userWallet, "main_wallet") ||
      0;

    const withdrawTransections = await FundTransaction.find({
      uCode: user._id,
      txType: "withdrawal",
    });
    ...
    withdrawTransections.forEach((item) => {
      if (item.status === 0) {
        withdrawAmount.pendingAmount += item.amount;
      } else if (item.status === 1) {
        withdrawAmount.receivedAmount += item.amount;
      } else if (item.status === 2) {
        withdrawAmount.rejectedAmount += item.amount;
      }
    });
    withdrawAmount.totalAmount =
      withdrawAmount.pendingAmount +
      withdrawAmount.receivedAmount +
      withdrawAmount.rejectedAmount;
...
const getCurrentWalletBalance = async (req, res) => {
  const postData = req.body;
  const vsuser = req.vsuser;
  try {
    const validateFields = ["walletType"];
...
async function handler(req, res) {
  ...
  if (req.method === "POST") {
    switch (userSlug) {
      case "getProfile":
        await routeHandler.getProfile(req, res);
        break;
      case "updateProfile":
      ...
      case "deleteInvoiceFile":
        ...
        break;
      case "getUserDashboardAnalytics":
        await routeHandler.getUserDashboardAnalytics(req, res);
        break;
      case "getCurrentWalletBalance":
        await getCurrentWalletBalance(req, res);
        break;
      default:
        routeFlag = false;
    }
