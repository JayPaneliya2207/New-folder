...
routeHandler.getUser = async (req, res) => {
  let postdata = req.body;
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      const userId = postdata.id;
      const user = await Users.findOne({ _id: userId }).catch((error) => {
        res.json({
          error: error,
          message: "There was an error!",
          status: "error",
      ...
      if (userdata) {
        ...
        if (userdata.validityDate && userdata.validityDate !== "") {
          ...
          await Users.findOneAndUpdate(
            { _id: userdata._id },
            { validityDate: newDate, planName: plandata.planname }
          );
        } else {
          validityDate = addDays(new Date(), plandata.validity);
          await Users.findOneAndUpdate(
            { _id: userdata._id },
            { validityDate: validityDate, planName: plandata.planname }
          );
        }
      }
    }
  ...
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      ...
      let settingdata = {
        ...
        paypalKey: postdata.paypalKey,
        paypalSecret: postdata.paypalSecret,
        currency: postdata.currency,
        isEnabled: postdata.isEnable,
        isAdEnabled: postdata.isAdEnable,
        paypalTestMode: postdata.paypalTestMode,
        isEnableBankTransfer: postdata.isEnableBankTransfer,
        bankTransferIns: postdata.instructions,
        razorpayKey: postdata.razorpayKey,
        razorpaySecret: postdata.razorpaySecret,
      };
    ...
    if (req._IS_ADMIN_ACCOUNT) {
      if (typeof postdata.id != "undefined" && postdata.id != "") {
        ...
        if (userExists && typeof userExists._id != "undefined") {
          res.json({
            status: "error",
            message: "The email already exist. Please use another email.",
          });
        } else {
          let access_level = [];
          let newUser = {
  ...
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      if (typeof postdata.id != "undefined" && postdata.id != "") {
        ...
        if (typeof postdata.price != "undefined" && postdata.price != "") {
        ...
        if (planExists && typeof planExists._id != "undefined") {
          res.json({
            status: "error",
            message: "The plan already exist. Please use another plan name.",
          });
        } else {
          let newPlan = {
            source: "Admin",
  ...
  try {
    if (typeof postdata.id != "undefined" && postdata.id != "") {
      ...
      if (categoryExists && typeof categoryExists._id != "undefined") {
        res.json({
          status: "error",
          message: "Category name already exist. Please use another name.",
        });
      } else {
        let newCat = {
          title: postdata.name,
...
routeHandler.createTemplate = async (req, res) => {
  ...
  try {
    ...
    if (
      typeof postdata.html_theme_id != "undefined" &&
      postdata.html_theme_id != ""
    ) {
      ...
      if (catData && typeof catData._id != "undefined") {
        ...
        if (templateExists && typeof templateExists._id != "undefined") {
          ...
          await Templates.create(fields).then(async (template) => {
            ...
            await TemplatePage.create(pageFields).then(async (templatePage) => {
...
routeHandler.getTemplatePages = async (req, res) => {
  ...
  try {
    ...
    let totalTemplatePages = await TemplatePage.find(query).countDocuments();
...
routeHandler.deleteTemplate = async (req, res) => {
  ...
  try {
    ...
    let shoulddelete = await CommonAPI.getAllkeysOfFolder(templateId);
    ...
    if (response.status) {
      ...
      if (themeData && typeof themeData._id != "undefined") {
        res.json({
          status: "error",
          message:
            "Theme is already exist with this name. Please use another name.",
        });
      } else {
        let fields = {
          userId: user._id,
  ...
  try {
    ...
    if (response.status) {
      ...
      if (themeData && typeof themeData._id != "undefined") {
        ...
        if (postdata.title) {
          let themeName = postdata.title.trim();
          let themeNameData = await Theme.findOne({
            title: themeName,
            _id: { $ne: postdata.id },
          });
...
routeHandler.addSocialType = async (req, res) => {
  ...
  try {
    ...
    if (response.status) {
      ...
      if (socialData && typeof socialData._id != "undefined") {
        res.json({
          status: "error",
          message:
            "Social type is already exist with this name. Please use another name.",
        });
      } else {
        let fields = {
          userId: user._id,
    ...
    if (response.status) {
      ...
      if (socialPackData && typeof socialPackData._id != "undefined") {
        res.json({
          status: "error",
          message:
            "Social pack is already exist with this name. Please use another name.",
        });
      } else {
        let fields = {
          userId: user._id,
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "public/site"); // Destination folder
  },
  filename: function (req, file, cb) {
    const ext = path.extname(file.originalname); // Get file extension
    const uniqueName = `${Date.now()}_${Math.round(Math.random() * 1e9)}${ext}`; // Create unique file name
...
routeHandler.uploadFile = async (req, res) => {
  upload.fields([
    { name: "logoDark" },
    { name: "logoWhite" },
    { name: "favicon" },
...
// Route handler to update frontend settings and handle file upload
routeHandler.updateFrontendSettings = async (req, res) => {
  upload.fields([
    { name: "logoDark" },
    { name: "logoWhite" },
    { name: "favicon" },
    ...
    const settingsData = {
      ...
      siteDescription: postdata.siteDescription,
      contactEmail: postdata.contactEmail,
      mobile: postdata.mobile,
      logoDark:
        req.files && req.files["logoDark"]
          ? `/site/${req.files["logoDark"][0].filename}`
          : postdata.logoDark,
      logoWhite:
        req.files && req.files["logoWhite"]
          ? `/site/${req.files["logoWhite"][0].filename}`
          : postdata.logoWhite,
      favicon:
        req.files && req.files["favicon"]
          ? `/site/${req.files["favicon"][0].filename}`
          : postdata.favicon,
    };

    // Continue saving or updating your settings as needed
    let where = { parentId: postdata.user_id };
    let Exists = await AdminSettings.find(where);

    if (Exists[0]) {
      console.log(settingsData);
      await AdminSettings.updateOne(where, {
        $set: { frontendSettings: settingsData },
      });
      res.json({
        status: "success",
      ...
      await AdminSettings.create({
        ...
        frontendSettings: settingsData,
      });
      res.json({
        status: "success",
        message: "Frontend settings created successfully.",
      });
    }
  });
};
  ...
  try {
    if (req._IS_ADMIN_ACCOUNT) {
      if (typeof postdata.id != "undefined" && postdata.id != "") {
        ...
        if (
          typeof postdata.discountType != "undefined" &&
          postdata.discountType != ""
        ) {
        ...
        if (
          typeof postdata.discount != "undefined" &&
          postdata.discount != ""
        ) {
          set.discount = postdata.discount;
        }
        if (
          typeof postdata.duration != "undefined" &&
          postdata.duration != ""
        ) {
          set.duration = postdata.duration;
        }
        if (
          typeof postdata.minAmount != "undefined" &&
          postdata.minAmount != ""
        ) {
...
routeHandler.getPendingPayments = async (req, res) => {
  try {
    let where = { paymentMethod: "Bank Transfer", paymentStatus: "Pending" };
    let sortData = { createdAt: -1 };

    let payments = await BillingModel.find(where)
      .populate("customer_id", "name email")
      .sort(sortData);
    let totalcount = await BillingModel.find(where).countDocuments();
    if (payments) {
      res.json({
        status: "success",
        message: "",
...
routeHandler.updatePaymentStatus = async (req, res) => {
  try {
    let id = req.body.payment_id;
    let payment = await BillingModel.findOne({ _id: id });
    if (payment) {
      let userid = payment.customer_id;
      await BillingModel.findOneAndUpdate(
        { _id: id },
        { paymentStatus: "Approved" }
      );

      let plan = await Plans.findOne({ _id: payment.planId });
      let userdata = await Users.findOne({ _id: userid });
      if (userdata) {
        let validityDate;
        if (userdata.validityDate && userdata.validityDate !== "") {
          validityDate = userdata.validityDate;
          let newDate = addDays(new Date(validityDate), plan.validity);
          await Users.findOneAndUpdate(
            { _id: userid },
            { validityDate: newDate }
          );
        } else {
          validityDate = addDays(new Date(), plan.validity);
          await Users.findOneAndUpdate(
            { _id: userid },
  ...
  if (req.method === "POST") {
    switch (adminSlug) {
      case "getUsers":
        ...
        break;
      case "getUser":
        await routeHandler.getUser(req, res);
        break;
      case "addCoupon":
      ...
      case "getPendingPayments":
        await routeHandler.getPendingPayments(req, res);
        break;
      case "updatePaymentStatus":
        await routeHandler.updatePaymentStatus(req, res);
        break;
      case "updateFrontendSettings": // Add this case for frontend settings
        await routeHandler.updateFrontendSettings(req, res);
        break;
      case "uploadFile":
        await routeHandler.uploadFile(req, res);
        break;
      default:
        routeFlag = false;
    }
