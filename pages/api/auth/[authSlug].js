...
routeHandler.login = async (req, res) => {
  ...
  if (!user) {
    res.json({
      status: "error",
      message:
        "Couldn't find a PixaURL account associated with this email and password. Please try again.",
    });
  } else {
    if (user.status) {
      if (user?.profilePicture != "") {
        try {
          ...
          console.error(`Error updating image at index :`, error);
        }
      }
      ...
      jwtdata.profile_img =
        user.profilePicture == undefined ? "" : user.profilePicture.file;
...
routeHandler.adminLogin = async (req, res) => {
  let postdata = req.body;
  let data = {
    email: postdata.email,
    password: md5(postdata.password),
  ...
  let user = await AdminUser.findOne(data).catch((error) => {
    res.json({
      status: "error",
      message: "There was an error!",
      d: error,
  ...
  if (!user) {
    ...
    if (user.status) {
      ...
      jwtdata.email = user.email;
      jwtdata.user_id = user._id;
      jwtdata.name = user.username || "Admin";
      jwtdata.profile_img =
        user.profilePicture == undefined ? "" : user.profilePicture.file;
      jwtdata.short_nm = user.username || "";
      jwtdata.role = user.role;
      jwtdata.createdAt = user.createdAt;
      let url = "";
...
function generateUsername() {
  const randomNumber = Math.floor(1000000 + Math.random() * 9000000); // Generates 7-8 digit number
  return `MX${randomNumber}`;
}

routeHandler.register = async (req, res) => {
  const postdata = req.body;
  try {
    let parentUser = await AdminUser.findOne({
      role: 1,
    }).catch(() => {
      res.json({
        message: "There was an error!",
    ...
    if (!user) {
      let uniqueUsername = generateUsername();
      let newUser = {
        source: "Self",
      ...
      Users.create(newUser).then(async (user) => {
        ...
        fs.readFile(
          ...
          async (err, htmlToSend) => {
            ...
            if (emailData?.emailSettings.name == "Mandrill") {
              mailResponse = await CommonAPI.sendMailUsingMandrill(
                params,
                settings
              );
            }
            if (emailData?.emailSettings.name == "SMTP") {
              mailResponse = await CommonAPI.sendMailUsingSMTP(
                params,
                settings
              );
            }
            if (emailData?.emailSettings.name == "Sendgrid") {
              mailResponse = await CommonAPI.sendMailUsingSendgrid(
                params,
                settings
              );
            }
            ...
            async (err, htmlToSend) => {
              ...
              if (mailResponse.accepted.length > 0) {
                ...
                res.json({
                  status: "error",
                  message:
                    "Unable to send password reset link on your email, Please try again.",
                });
              }
            }
          );
...
routeHandler.getPlans = async (req, res) => {
  ...
  try {
    ...
    if (postdata.searchTerm && postdata.searchTerm !== "") {
      ...
      query = {
        $or: [
          ...
          {
            price: /^\d+$/.test(postdata.searchTerm)
              ? parseInt(postdata.searchTerm)
              : null,
          },
          {
            validity: /^\d+$/.test(postdata.searchTerm)
              ? parseInt(postdata.searchTerm)
              : null,
          },
        ],
    ...
    let stripeActive,
      ...
      razorpayActive;
    if (adminSetting) {
      if (adminSetting.stripeKey != "") {
        stripeActive = true;
      }
      if (adminSetting.paypalKey != "") {
        paypalActive = true;
      }
      if (adminSetting.razorpayKey != "") {
        razorpayActive = true;
      }
      if (
        adminSetting.isEnableBankTransfer == true &&
        adminSetting.bankTransferIns !== ""
      ) {
        bankTransferActive = true;
        bankTransferIns = adminSetting.bankTransferIns;
      }
    ...
    res.json({
      ...
      bankTransferIns: bankTransferIns,
...
routeHandler.getCoupons = async (req, res) => {
  try {
    ...
    if (postdata.couponCode) {
      let couponExists = await Coupons.findOne({
      ...
      if (couponExists != null) {
        ...
        if (couponExists.duration == "Once per user") {
          let user = await BillingModel.findOne({
    ...
    if (postdata.searchTerm && postdata.searchTerm !== "") {
      ...
      query = {
        $or: [
          ...
          {
            couponName: /^\d+$/.test(postdata.searchTerm)
              ? parseInt(postdata.searchTerm)
              : null,
          },
          {
            couponCode: /^\d+$/.test(postdata.searchTerm)
              ? parseInt(postdata.searchTerm)
              : null,
          },
        ],
...
async function handler(req, res) {
  ...
  if (req.method === "POST") {
    switch (authSlug) {
      case "adminLogin":
        await routeHandler.adminLogin(req, res);
        break;
      case "login":
