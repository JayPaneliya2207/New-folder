...
const Billings = () => {
  ...
  const handleDownloadFile = (filedata) => {
    fetch(`${process.env.APP_URL.concat(`/${filedata.filePath}`)}`).then(
      (response) => {
  ...
  if (orderList.length) {
    orderList.forEach((item, index) => {
      const newItem = {
        ...
        couponCode:
          item.couponCode != undefined && item.couponCode != ""
            ? item.couponCode
            : "-",
        ...
        <div className="pu_datatable_wrapper">
          <div className="pu_pagetitle_wrapper">
            <div>
              ...
              {expiresDate ? (
                <h4>
                  ...
                  {moment(expiresDate).format("DD-MMM-YYYY")} )
                </h4>
              ) : (
                ""
              )}
            ...
            <div className="pu_pagetitle_right">
              <div className="pu_search_wrapper"></div>
            </div>
          </div>
          <DataTable
            columns={columns}
